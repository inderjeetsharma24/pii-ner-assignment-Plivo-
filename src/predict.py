...
def bio_to_spans(text, offsets, label_ids):
    ...
    for (start, end), lid in zip(offsets, label_ids):
        ...
        if prefix == "B":
            ...
            # Handle I-tag without preceding B-tag (treat as B)
            if current_label == ent_type:
                current_end = end
            else:
                # New entity starting with I-tag (shouldn't happen but handle gracefully)
                if current_label is not None:
                    spans.append((current_start, current_end, current_label))
                current_label = ent_type
                current_start = start
    ...
    with open(args.input, "r", encoding="utf-8") as f:
        for line in f:
            ...
            for s, e, lab in spans:
                # Calculate confidence for this span
                span_indices = [i for i, ((start, end), _) in enumerate(zip(offsets, pred_ids))
                               if start >= s and end <= e and start != 0 and end != 0]
                if span_indices:
                    max_conf = max(probs[i, pred_ids[i]].item() for i in span_indices)
                    # Filter by confidence threshold (helps with precision)
