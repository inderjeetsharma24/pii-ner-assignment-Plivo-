...
class PIIDataset(Dataset):
    def __init__(self, path: str, tokenizer, label_list: List[str], max_length: int = 256, is_train: bool = True):
        ...
        # Data augmentation for small datasets - duplicate samples multiple times
        # This helps the model see the same patterns more often
        if is_train and len(items_raw) < 10:
            augmented = []
            # Repeat each sample multiple times to give model more exposure
            for obj in items_raw:
                for _ in range(12):  # 12 copies = 24 total from 2 samples (more exposure)
